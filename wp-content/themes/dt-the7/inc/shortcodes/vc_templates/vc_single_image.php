...
if ( $img_link_large == true ) {
	...
	$link_to = $link_to[0];
	$a_class = ' class="rollover rollover-zoom dt-mfp-item mfp-image dt-single-mfp-popup ' . $img_class . ' ' . $style . '"';
} else if ( strlen($link) > 0 ) {
	$link_to = $link;
	$a_class = ' class="rollover ' . $img_class . ' ' . $style . '"';
...
if ( $img == NULL ) $img['thumbnail'] = '<img class="vc_img-placeholder ' . $img_class . '" src="' . vc_asset_url( 'vc/no_image.png' ) . '" />'; //' <small>'.__('This is image placeholder, edit your page to replace it.', LANGUAGE_ZONE).'</small>';
